---
title: 芝麻代理 代理服务情况与使用参考
description: 面向 国内用户的 芝麻代理 代理服务资料整理：提供 residential、datacenter 代理，覆盖 多地区，支持 http、https。资料源于官方公开信息。
pageType: providers
provider: 芝麻代理
status: inactive
officialUrl: "待核实"
proxyTypes:
  - residential
  - datacenter
protocols:
  - http
  - https
authentication:
  - username-password
  - api
  - ip-whitelist
pricingModels:
  - traffic
  - package
regions: ""
summary: 芝麻代理 提供 residential/datacenter代理，主要面向sme、enterprise，覆盖 待核实；本文基于官方公开资料整理，具体能力以官方页面为准。
takeaway: 芝麻代理 是否适合你的场景，需结合覆盖区域、认证方式与计费模式综合判断；具体以下单页面为准。
author: EasyBR 团队
updatedAt: 2026-08-08
disclosure: 本服务商已停止（或停止提供）代理 IP 服务，本页保留作为历史记录；请以其他在营服务商为准。
sidebar:
  hidden: true

---

> 本服务商已停止（或停止提供）代理 IP 服务，本页保留作为历史参考；请以其他在营服务商为准。

## 一句话结论

芝麻代理 是面向国内用户的住宅代理服务商。资料基于官方公开页面整理，未做主观评分。

## 基本信息

- 官网：待核实
- 更新日期：2026-08-08
- 状态：已停止服务（历史条目）

## 提供哪些代理产品

- residential
- datacenter

## 覆盖区域

待核实，以官方页面为准

## 认证和连接方式

- username-password
- api
- ip-whitelist

## 计费方式

- traffic
- package

## 主要优点

- 官方公开资料中的能力描述以官方页面为准；本页不做主观评分与夸大宣称。

## 需要注意的限制

- 限制信息以官方说明为准；使用前建议阅读服务条款与适用地区说明。

## 官方入口与文档

- 官网入口：待核实


## 信息来源

- 暂无外部来源记录

## 同类服务商

可在本栏目浏览其他服务商对比（仅链已发布页面）。

## 使用代理的配套工具

EasyBR 指纹浏览器支持 HTTP/HTTPS/SOCKS5 代理配置，可将本页列出的代理按其认证方式配置后使用；更多说明见「EasyBR 工具」栏目。
